...
const WelcomeSectionTextSchema = z
    .object({
        ...
        contentTitle: z.string().trim().min(1, 'Content title is required').max(100).optional(),
    ...
    try {
        ...
        if (contentType.includes('application/json')) {
            ...
            if (!result.success) {
                ...
                ({title, subtitle, contentTitle, contentSubtitle} = result.data);
            ...
            const result = WelcomeSectionTextSchema.safeParse({title, subtitle, contentTitle, contentSubtitle});
            if (!result.success) {
                return NextResponse.json({errors: result.error.errors}, {status: 400});
            }
            ({title, subtitle, contentTitle, contentSubtitle} = result.data);
        ...
        if (Object.keys(updateData).length === 0) {
            return NextResponse.json(
                {error: 'You must send at least one text field or an image to update.'},
                {status: 400}
            );
        }
